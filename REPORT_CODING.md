...
**Date**: 2026-09-06 (v5.4 — aspect ratio COMPLETED · LBM independent validation integrated & running)
...
**Next methodological step (frozen)**: independent optimization / LBM reference (`14_independent_optimization`, R2-1/R2-3/R3-3). **LBM now integrated** (`lbm_mrt_validation/`) and the full independent-validation campaign is running (2026-09-06). `aspect_ratio` (R3.2) has been **COMPLETED**.
...
### Results (COMPLETED 2026-09-06 — Re=500, seed 42, E*=0.25, Fourier basis)

| Metric | Square (1:1) | Rectangular (2:1) |
|---|---|---|
| `E_total` (quadrature, coverage=1.0) | 0.2534 | 0.2514 |
| A20 (`c_{1,0}`, mode sin(2πx/Lx)) | 0.6835 | 0.6770 |
| A10 / A30 | 0.0011 / −0.0101 | 0.0135 / −0.0596 |
| **mode2_fraction** f₂ | **0.9220** | **0.9118** |
| temporal_fraction f_t | 0.0125 | 0.0158 |
| dominant (i,j) | (1,0) | (1,0) |
| Control dissipation proxy D_ctrl = (1/Re)∫U'² dx | 0.0185 | 0.0092 (per unit area 0.0046) |
| `reconstruction_rmse` | 0.0 | 0.0 |

**Verdict — Case A (geometric robustness)** : the mode-2 collapse **persists** in the 2:1 geometry (f₂ = 91.2 % vs 92.2 %; A20 unchanged to <1 %; E at target). R3's request not to over-generalize the phenomenon is answered **experimentally**: the phenomenon is robust to the Lx:Ly aspect ratio tested (1:1 and 2:1), while the parametrization dependence remains (f₂ = 92 % in Fourier vs 6 % in Chebyshev at the same Re — campaign 13). Output: `09_aspect_ratio/aspect_ratio_results.csv`.
...
## 18bis. LBM independent validation (R2-1/R2-3/R3-3) — INTEGRATED (2026-09-06)

**Status**: solver integrated in `lbm_mrt_validation/lbm_mrt_pinn_validation.py` (LBM-MRT D2Q9, Beniaiche et al. 2026); full campaign RUNNING (launched 2026-09-06 08:39).

### Scientific question (clean wording)

> *Does the control identified by the PINN remain **physically performant when injected into an independent Navier–Stokes solver**?*

Not "prove global optimality". Two tests:

1. **Control injection**: the identified `U_lid(x,t,Re)` (campaign 13, seed 42) is injected as the lid velocity in the LBM-MRT solver.
2. **Branch admissibility**: both branches are validated independently —
   - **Fourier / mode-2 branch** (quasi-stationary, f_t ≈ 1 %) → stationary + full time-dependent forcing.
   - **Chebyshev_mod / temporal branch**(0,1), f_t ≥ 93 %) → **time-dependent forcing only** (a static lid cannot represent it; the extended kernel `mrt_step` accepts `U_lid2d[t_idx, :]`, one forcing period = `NT_PER=256` steps, natural period 2 of `cos(jπt)`).

### Physics / numerics of the LBM run

- LBM-MRT D2Q9, half-way bounce-back (PULL), s_e=s_eps=1.4, s_q=1.2, ν = U_ref·N/Re with U_ref=0.05 (Ma ≈ 0.09), Numba parallel on all cores.
- **Strict separation**: the NPZ control series are generated by `make_pinn_lid_series.py` (Python 3.14 + torch, **only** the PINN env) — the LBM (Python 3.11 + numba) only reads tabulated controls. `E_period` of the generated series matches `E_total` of campaign 13 (fourier 0.258/0.253/0.269 — cheb 0.289/0.285/0.291) and continuity over one period = 0.
- **Metrics** on the time-averaged response field: ε (viscous dissipation), K (kinetic energy), Z (enstrophy) + **K_fluct** = ∫½(u′²+v′²) dΩ (weight of temporal fluctuations) + modal analysis of `u_mid(y)` (mean) and of `u_mid_rms(y)` (fluctuation field, meaningful when the mean ≈ 0).
- Protocol (publication grade): Ghia N=256 (Re 100, 1000); GCI on [128, 256, 512] at Re=500, sin(2πx); 12 production runs (4 profiles × 3 Re) at N=256, max_iter=2·10⁵; 6 temporal branch runs (`pinn_t`, `cheb_t` × 3 Re).

### Early smoke-test signal (N=96, Re=500, 8192 iters)
...
| **Strongly recommended** | Aspect ratio | ✅ **Run** (R3.2 closed: mode-2 persists 1:1 vs 2:1, f₂ 0.922 vs 0.912) |
| **Strongly recommended** | Unseen-Re | 🟡 Implemented (interpolation, not exhaustive validation) |
| **Strongly recommended** | Convergence/uncertainty | ✅ Implemented (computational robustness, NOT a GCI — GCI now furnished on LBM side) |
...
| **Expensive/later** | Independent optimization | 🟢 **LBM integrated & validation campaign running** (18bis) ; traditional optimization = placeholder |
