...
\title{\large Supplementary material:\\ Physics-informed identification and independent numerical verification of conditional modal collapse in active control of lid-driven cavity flow}

\author{Ahmed Beniaiche, Abderrahim Larabi, Abderrahim Dourari, and Mbarek Belkadi}

\date{}

\footnotetext{Ahmed Beniaiche and Abderrahim Larabi: Laboratory of Fluid Mechanics, \'Ecole Militaire Polytechnique, Algiers, Algeria.}

\footnotetext{Abderrahim Dourari and Mbarek Belkadi: Laboratory of Turbomachinery, \'Ecole Militaire Polytechnique, Algiers, Algeria.}

\footnotetext{Corresponding author: \texttt{beniaiche\_ahmed25@yahoo.fr}.}

\begin{document}

\maketitle

This document collects the supplementary figures and tables that support the main text. Section, figure, and equation references that contain a number correspond to the main text, and are resolved through the cross-referencing mechanism of the accompanying files.

\section{Network architecture sweep}\label{ssec:archtable}

Table~S1 complements the architecture-robustness table of Section~\ref{sec:archmode} of the main text with the realized energies, parameter counts, and temporal fractions of the small and large networks. All runs are at $\re=500$ with the Fourier basis, three independent seeds per architecture, and $E^*=0.25$. The seed of the baseline network that coincides with the intermediate branch of the main-text seed study (Section~\ref{sec:seeds}) is excluded from the reported range.
...
Figures~S1 and~S2 show the velocity magnitude and the viscous dissipation fields of the controlled flow at $\re=100$, $500$, and $1000$, for the Fourier branch recovered in the main text, as realized by the independent LBM solver. The dissipation-density field $\Phi$ of Eq.~\eqref{eq:phi} of the main text is displayed. The central circulation produced by the dominant second spatial mode and the corresponding dissipation concentration are apparent.
